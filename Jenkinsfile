...
node() {
        ...
        [
            ...
            parameterDefinitions: [
                ...
                choice(
                    ...
                    description : 'Multi-arch payloads are now generated by build-sync-multi pipeline. "auto" sets to false for stream/test assemblies, true otherwise. Set to "false" only if you need legacy multi-arch payload generation behavior.',
    ...
    stage("Initialize") {
        ...
        // Resolve SKIP_MULTI_ARCH_PAYLOAD: "auto" means multi-payload generation will be skipped
        // for stream and test assemblies. Multis for stream are generated by the multi-build-sync job.
        // For named assemblies (4.y.z), multi creation should not be skipped.
        if (params.SKIP_MULTI_ARCH_PAYLOAD == 'auto') {
            skipMultiArchPayload = (params.ASSEMBLY == 'stream' || params.ASSEMBLY == 'test')
        } else {
            skipMultiArchPayload = (params.SKIP_MULTI_ARCH_PAYLOAD == 'true')
        }
        echo("SKIP_MULTI_ARCH_PAYLOAD resolved to: ${skipMultiArchPayload} (param was: ${params.SKIP_MULTI_ARCH_PAYLOAD}, assembly: ${params.ASSEMBLY})")

        ...
        script {
            ...
            buildlib.withAppCiAsArtPublish() {
                commonlib.shell(
                    script: """
                        if [[ -f /bin/scl ]]; then
                        scl enable rh-python38 -- python3 release-tool.py --message "${message}" --reason "${message}" --architecture ${params.ARCH} --kubeconfig ${env.KUBECONFIG} ${confirm_param} ${action} ${params.RELEASE_NAME}
                        else
                        python3 release-tool.py --message "${message}" --reason "${message}" --architecture ${params.ARCH} --kubeconfig ${env.KUBECONFIG} ${confirm_param} ${action} ${params.RELEASE_NAME}
                        fi
                        """,
                )
            }
